...
Large raw simulation outputs are reproducible from the scripts in `code/` and are not all tracked in Git. The final publication tables are tracked here so readers can audit the values reported in the paper. Figure provenance is listed in `figures/figure_order.tsv`; Figure S1 is generated by `code/31_plot_ordinal_power_5pt_vs_6pt.R`.
