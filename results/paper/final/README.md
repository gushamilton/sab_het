...
This directory contains the publication-facing figures and tables for the SAB heterogeneous treatment effect simulation paper.

## Scope

- `figures/`: final figure PDFs used for the manuscript.
- `tables/`: final manuscript and supplementary tables, plus `supplementary_tables.xlsx`.
...
The committed final assets match the submission manuscript values for the main numerical claims:
...
Large raw simulation outputs are reproducible from the scripts in `code/` and are not all tracked in Git. The final publication tables and figures are tracked here so readers can audit the values reported in the paper.
