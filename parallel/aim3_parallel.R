...
if (!is.na(summary_result$nns_needed)) {
  ...
  detailed_filename <- sprintf("%s/aim3_detailed_scenario_%03d.tsv", output_root, scenario_id)
  write_tsv(detailed_results, detailed_filename)
  message(paste("Detailed results saved to:", detailed_filename))
}

# Save summary results to file
summary_filename <- sprintf("%s/aim3_summary_scenario_%03d.tsv", output_root, scenario_id)
