...
power_aim1 <- read_csv("../results/tables/aim1_power_summary.csv")
plot_aim1 <- readRDS("../results/objects/aim1_power_plot.rds")

summary_aim2 <- read_csv("../results/tables/aim2_accuracy_summary.csv")
plot_aim2_power <- readRDS("../results/objects/aim2_power_plot.rds")
plot_aim2_bias <- readRDS("../results/objects/aim2_bias_plot.rds")
plot_aim2_wrong_dir <- readRDS("../results/objects/aim2_wrongdir_plot.rds")

summary_aim3 <- read_csv("../results/tables/aim3_nns_summary.csv")
plot_aim3_nns <- readRDS("../results/objects/aim3_nns_plot.rds")
...
summary_aim3 %>%
  mutate(
    `NNS` = if_else(is.na(nns_needed), "> 500,000", scales::comma(nns_needed)),
    `NNR` = if_else(is.na(nnr_corresponding), "-", scales::comma(round(nnr_corresponding), 1))
  ) %>%
  select(Subgroup = target_group, Accuracy = accuracy, NNS, NNR) %>%
  gt(groupname_col = "Subgroup") %>%
  fmt_number(columns = Accuracy, decimals = 2)

#| label: fig-aim3-plot-nns
#| fig-cap: "Aim 3: Number Needed to Screen (NNS) vs. Classification Accuracy"
plot_aim3_nns
```
...
```{r visualization_bias2_supp, echo=FALSE}
...
```{r visualization_bias2_realistic_supp, echo=FALSE}
