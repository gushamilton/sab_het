...
# For local execution, set the working directory to the paper folder if needed:
if (basename(getwd()) != "paper" && dir.exists("paper")) {
  setwd("paper")
}
...
3.  **Outcome Generation:** A binary outcome ('success', representing death by 84 days = 1, survival = 0) was generated based on the patient's true subgroup, treatment assignment, the subgroup-specific baseline event probability (`p0_vector`, defined using overall mortality across both trial arms), and the subgroup-specific treatment effect (odds ratio, `or_vector`). The probability of death for patient `i` in subgroup `j` receiving treatment `t` (0 or 1) was calculated using the logistic model: $P(\text{Death}_{ij} | \text{Treatment}=t) = \text{expit}(\text{logit}(p0_j) + \log(OR_j) \times t)$. The outcome was then drawn from a Bernoulli distribution with this probability.

**Parameterization:** Two parameter sets were used: \* **ARREST Scenario:** Based on the Swets et al. [@Swets2024] analysis of the ARREST trial 84-day mortality data. \* `or_vector`: `c(A = 1.0, B = 18.8, C = 0.79, D = 1.4, E = 0.3)` \* `freq_vector`: `c(A = 115/758, B = 107/758, C = 276/758, D = 121/758, E = 139/758)` (overall prevalence across both arms) \* `p0_vector_adjusted`: `c(A = 25/115, B = 8/107, C = 53/276, D = 22/121, E = 4/139)` (overall mortality across both arms). \* **ARREST (shrunk) Scenario:** log-OR shrinkage (k=0.5) applied to subgroup ORs to temper winner’s-curse inflation.
...
power_aim1 <- read_tsv(file.path(results_root, "tables/aim1_power_summary.tsv"))
plot_aim1 <- readRDS(file.path(results_root, "objects/aim1_power_plot.rds"))

summary_aim2 <- read_tsv(file.path(results_root, "tables/aim2_accuracy_summary.tsv"))
plot_aim2_power <- readRDS(file.path(results_root, "objects/aim2_power_plot.rds"))
plot_aim2_bias <- readRDS(file.path(results_root, "objects/aim2_bias_arrest_plot.rds"))
plot_aim2_wrong_dir <- readRDS(file.path(results_root, "objects/aim2_wrong_dir_plot.rds"))

aim3_closed_form <- read_tsv(file.path(results_root, "tables/aim3_closed_form_summary.tsv"))
...
# Filter and format for ARREST_raw scenario, excluding group A
tbl_aim2_arrest <- summary_aim2 %>%
  filter(scenario_name == "ARREST_raw") %>%
...
plot_aim2_bias_a <- ggplot(
  summary_aim2 %>% filter(group != "Overall", group != "A", scenario_name == "ARREST_raw"),
...
### Aim 3: Sample Size Requirements (Enrichment Trial, Closed-Form)

```{r aim3_closed_form}
#| tbl-cap: "Aim 3: Closed-Form NNS/NNR by Test Type and Target Group"

aim3_closed_form %>%
  filter(target_group %in% c("B", "C", "D", "E")) %>%
  select(test_type, target_group, nns_closed_form, nnr_closed_form, predicted_beta, bias_closed_form) %>%
  ...
  fmt_number(columns = c(nns_closed_form, nnr_closed_form), decimals = 0) %>%
  fmt_number(columns = c(predicted_beta, bias_closed_form), decimals = 3)
```
...
## Appendix: Aim 3 Simulation Validation

```{r aim3_sim_table}
#| tbl-cap: "Aim 3: Simulation vs Closed-Form Validation (Supplementary)"

aim3_combined_gt_path <- file.path(results_root, "objects/aim3_combined_gt_table.rds")

if (file.exists(aim3_combined_gt_path)) {
  readRDS(aim3_combined_gt_path)
} else {
  cat("Simulation validation table is not available yet.\n")
}
```

```{r aim3_sim_nns}
#| fig-cap: "Aim 3: NNS Comparison - Simulation vs Closed Form"

aim3_nns_plot_path <- file.path(results_root, "objects/aim3_nns_comparison_plot.rds")
if (file.exists(aim3_nns_plot_path)) {
  readRDS(aim3_nns_plot_path)
} else {
  cat("NNS comparison plot is not available yet.\n")
}
```

```{r aim3_sim_nnr}
#| fig-cap: "Aim 3: NNR Comparison - Simulation vs Closed Form"

aim3_nnr_plot_path <- file.path(results_root, "objects/aim3_nnr_comparison_plot.rds")
if (file.exists(aim3_nnr_plot_path)) {
  readRDS(aim3_nnr_plot_path)
} else {
  cat("NNR comparison plot is not available yet.\n")
}
```
...
aim3_bias_plot_path <- file.path(results_root, "objects/aim3_bias_comparison_plot.rds")
if (file.exists(aim3_bias_plot_path)) {
  readRDS(aim3_bias_plot_path)
} else {
  cat("Bias comparison plot is not available yet.\n")
}
```

## References

<!-- references removed: figures/tables only -->
