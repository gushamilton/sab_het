...
# trial, without Monte-Carlo simulation.  The formula is a Wald approximation
# that incorporates dilution from misclassification.
# -----------------------------------------------------------------------------
...
# Prevalence and baseline risks (unchanged)
...
calc_required_nns <- function(beta_obs, enrol_rate,
  ...
  if (beta_obs == 0 || is.na(beta_obs) || enrol_rate == 0) return(Inf)
  # Wald variance approx:  var(beta) ≈ 4 / n_total (balanced arms, rare events)
  n_total <- (2 * (z_alpha_half + z_power_target) / abs(beta_obs))^2
