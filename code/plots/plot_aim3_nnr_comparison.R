# Plot Aim 3 NNR comparison (simulation vs closed form)
...
output_file <- file.path(results_root, "plots", "aim3_nnr_comparison.pdf")
...
nnr_plot <- combined %>%
  filter(target_group %in% c("B", "C", "D", "E"), is.finite(nnr)) %>%
  ggplot(aes(x = test_type, y = nnr, color = method, group = method)) +
  geom_line() +
  geom_point(size = 2) +
  facet_grid(scenario_name ~ target_group, scales = "free_y") +
  scale_y_log10(labels = scales::comma) +
  labs(
    title = "Aim 3: NNR Comparison (Simulation vs Closed Form)",
    x = "Test Type",
...
ggsave(output_file, nnr_plot, width = 12, height = 8)
