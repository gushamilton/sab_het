...
misclassify_group <- function(sim_data, accuracy, freq_vector, seed = NULL) {
  ...
  return(sim_data_with_assigned)
    ...
    #' @param true_overall_beta The pre-calculated true marginal log-OR for the overall population.
    #'
    ...
    replicate_sims <- function(or_vector, freq_vector, n, p0_vector = NULL, accuracy = 1, seed = 123, n_reps = 100, true_overall_beta = NA_real_) {
      ...
      purrr::map_dfr(1:n_reps, function(k) {
        ...
        estimate_effect_misclassify(misclassified_data, or_vector, accuracy = accuracy, true_overall_beta = true_overall_beta) |>
    ...
    #' @export
    estimate_effect_misclassify <- function(sim_data, or_vector, accuracy = 1, true_overall_beta) {
...
common_file <- common_opts[file.exists(common_opts)][1]
if (!is.na(common_file)) {
  source(common_file, local = FALSE)
